...
var GPTouchLayer = cc.Layer.extend({
    ...
    initGame: function() {
        ...
        this.lastMoveTime = 0; // Record last moving time to prevent moving too fast
    ...
    // BatchNode will improve rendering performance
    ...
    move: function(direction) {
        ...
        var self = this,
            cell, tile, score,
            vector, traversals, moved, now;
        
        now = Date.now();

        if (this.state === GC.GAME_STATE.OVER || (now - this.lastMoveTime) < 150)
            return;
